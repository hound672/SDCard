...
/**
	* @brief	Запись данных на SD карту
	* @param
	* @retval
	*/
int CSDCard::writeBlock(const u8 *data, u32 address)
{
	...
	SPI_SendRecvByte(spidevSDFlash, 0xFF); //один байт ожидания
	SPI_SendRecvByte(spidevSDFlash, 0xFE); //метка начала пакета данных

	u8 buf[dataBlockSize];
	memset(buf, 0xFF, dataBlockSize);
	SPI_SendRecv(spidevSDFlash, data, buf, dataBlockSize, false);

	SPI_SendRecvByte(spidevSDFlash, 0x00); // 1-ый байт CRC
	SPI_SendRecvByte(spidevSDFlash, 0x00); // 2-ой байт CRC
...
/**
	* @brief	Ожиданием окончания принятия команды
	* @param
	* @retval
	*/
